...
echo "Using arguments: spacing_ms=$ARG1, msg_size=$ARG2"
# Execute ROS2 node in interactive mode (not detached)
echo "Starting ROS2 node..."
docker exec -it "$CONTAINER_NAME" bash -c "source /home/testing/dev_ws/install/setup.bash && ros2 launch latency_test_listener listener_launch.py spacing_ms:=$ARG1 msg_size:=$ARG2" 
