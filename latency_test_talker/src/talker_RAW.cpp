...
void raw_sync_check(int sockfd, struct sockaddr_ll sll, unsigned char dest[6],
  ...
  char s_ACK[] = "S_ACK\0";
  size_t msg_len = strlen(s_ACK);
  memcpy(frame + sizeof(struct ethhdr), s_ACK, msg_len);
  ...
  while (true) {
    // Send s_ACK
    ssize_t sent = sendto(sockfd, frame, frame_len, 0, (struct sockaddr *)&sll,
                          sizeof(sll));
